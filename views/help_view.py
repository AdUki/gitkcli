"""
Help view for displaying keyboard shortcuts and commands
"""
import curses
from views.base_view import BaseView

class HelpView(BaseView):
    """View for displaying help information"""
    
    def __init__(self, stdscr):
        """
        Initialize help view
        
        Args:
            stdscr: Curses window object
        """
        super().__init__(stdscr)
        self.help_text = self._generate_help_text()
        
    def _generate_help_text(self):
        """Generate help text content"""
        return [
            "GITK CLI HELP",
            "",
            "Navigation:",
            "  j, DOWN    : Move down",
            "  k, UP      : Move up",
            "  h, LEFT    : Scroll left",
            "  l, RIGHT   : Scroll right",
            "  g          : Go to top",
            "  G          : Go to bottom",
            "  d, PgDn    : Page down",
            "  u, PgUp    : Page up",
            "",
            "Search:",
            "  /          : Start search",
            "  Tab        : Cycle search types (Message/Path/Content)",
            "  n          : Next search result",
            "  N          : Previous search result",
            "  ESC        : Cancel search",
            "  ENTER      : Complete search",
            "",
            "Search Types:",
            "  Message    : Search in commit messages, authors, and IDs",
            "  Path       : Search for commits touching specific paths",
            "  Content    : Search for commits adding/removing specific text",
            "  (All search types support regular expressions)",
            "",
            "Actions:",
            "  ENTER      : Show/hide diff for selected commit",
            "  c          : Copy commit ID to clipboard",
            "  r          : Refresh commit list",
            "  :          : Enter command mode",
            "",
            "Command Mode:",
            "  Enter a git command to execute",
            "  The current commit ID will be automatically added to the end of the command",
            "  Example: git show (will run: git show <current_commit_id>)",
            "  ENTER      : Execute command",
            "  ESC        : Cancel command",
            "",
            "Diff View:",
            "  j, DOWN    : Move down",
            "  k, UP      : Move up", 
            "  d, PgDn    : Page down",
            "  u, PgUp    : Page up",
            "  /          : Search in diff",
            "  n          : Next search result",
            "  N          : Previous search result",
            "  g          : Go to top of diff",
            "  G          : Go to bottom of diff",
            "  ENTER      : Return to commit list",
            "  b          : Show origin of the line at cursor",
            "",
            "Line Origin:",
            "  ENTER      : Jump to the origin commit",
            "  Any key    : Close the popup",
            "",
            "Views:",
            "  H          : Show/hide this help",
            "  q          : Quit",
            "",
            "Press any key to close help"
        ]
        
    def draw(self):
        """Draw the help view"""
        # Draw each line of help text
        for i, line in enumerate(self.help_text):
            if i >= self.max_lines:
                break
                
            # First line is header
            attr = curses.color_pair(10) if i == 0 else curses.color_pair(1)
            
            try:
                self.stdscr.addstr(i, 0, line.ljust(self.max_cols - 1), attr)
            except curses.error:
                # Ignore potential curses errors
                pass
                
    def handle_key(self, key):
        """
        Handle key press for help view - any key returns to previous view
        
        Args:
            key: Key code
            
        Returns:
            tuple: (continue_program, switch_view, view_name)
        """
        # Handle window resize to redraw help
        if key == curses.KEY_RESIZE:
            return True, False, None
        
        # Any other key returns to the previous view
        return True, True, "previous"
